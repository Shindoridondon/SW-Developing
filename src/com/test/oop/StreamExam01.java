...
public class StreamExam01 {
    ...
    public static void main(String[] args) {
        ...
        //이 값들을 10배 증가 시킨 결과물을 리스트 형태로 출력
     ...
     */
     ...
     /*
        ...
        - 이러한 중개 연산은 스트림을 전달 받아 스트림을 반환하므로, 중개 연산을 연속으로 연결
        - 스트림의 중개 연산은 필터-맵(filter-map) 기반의 API를 사용함으로써 지연(lazy) 연산
      ...
      */
      ...
      System.out.println("<!--- 스트림 변환 : 문자열로 이루어진 스트림을 map() 메소드를 이용하여 각 문자열의 길이를 이루어진 스트림으로 변환 --->");
      Stream<String> stream4 = Stream.of("HTML", "CSS", "JAVA", "JAVASCRIPT");
      stream4.map(s->s.length()).forEach(System.out::println);

      System.out.println("<!--- 스트림 변환 : 여러 문자열이 저장된 배열을 각 문자열에 포함된 각 스트림으로 변환 --->");      
      ...
      //limit() : 해당 스트림의 첫번째 요소부터 n만큼의 요소만으로 이루어진 새로운 스트림 생성 1 <= n
      //skip() : 해당 스트림의 첫번째 요소부터 n만큼의 요소를 제외한 나머지 요소만으로 이루어진 새로운 스트림 생성 n <= 끝
      
      IntStream stream6 = IntStream.range(0,10); //0부터 시작된 10개의 정수값을 리턴
      
      stream6.skip(4).forEach(n->System.out.print(n + " "));

      IntStream stream7 = IntStream.range(0,10); //0부터 시작된 10개의 정수값을 리턴
       ...
       *                  연산을 수행한 뒤, 첫번째 및 두번째 요소의 값을 삭제시키고,
       *                  그 결과와 세번째 요소를 가지고 또다시 연산을 수행.
       *                  이런 식으로 해당 스트림의 모든 요소를 차례차례 소모하면서
       *                  연산을 수행하고 그 결과를 반환
       ...
       */
       ...
       System.out.println("<!--- reduce() 메소드를 이용하여 1부터 100까지의 합 구하기 --->");
